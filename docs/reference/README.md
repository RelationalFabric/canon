---
title: Reference
---

# Reference

Authoritative specifications and integration guides for Canon's APIs and semantic building blocks.

- [API Reference](./api.md) — Canon package exports and runtime behaviors
- [Core Axioms](./axioms.md) — Detailed specifications for the foundational axioms
- [Canons Reference](./canons.md) — Guidance for implementing and extending canons
- [Augmentable Interfaces](./augmentable-interfaces.md) — Module augmentation surfaces for custom axioms and canons
- [Canon Kit & Third-Party Utilities](./kit.md) — Approved external dependencies and usage patterns
- [Technology Radar Utilities](./radar.md) — Conversion and validation helpers that back the radar build scripts
